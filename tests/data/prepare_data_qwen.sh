...
# Step 1: Download WikiText-103 and convert to JSONL (reuses cached download)
# Step 2: Tokenize and build Megatron mmap indexed dataset
# Output: ${DATA_DIR}/qwen_dataset_text_document.{bin,idx}
python3 -c "
import sys, json, os
...
output_prefix = os.path.join(DATA_DIR, 'qwen_dataset_text_document')
...
echo "Output files:"
ls -lh "${DATA_DIR}"/qwen_dataset_text_document.*
