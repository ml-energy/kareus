...
#                   nemo_experiments/<model>/<config_tag>/kareus/frontier/.
...
for i in "${!CONFIGS[@]}"; do
    ...
    mkdir -p "${RESULTS_DIR}" "${METHOD_DIR}"
    ...
    if [[ "${FRONTIER}" == "true" ]]; then
        ...
        if [[ "${SKIP_PROFILING}" == "false" ]]; then
            ...
            if [[ "${NODE_RANK}" == "0" ]]; then
                ...
                MODEL_NAME="${nemo_model_name}" CONFIG_TAG="${config_tag}" \
                    bash "${SCRIPT_DIR}/create_frontier.sh" kareus
            fi
        ...
        if [[ "${NODE_RANK}" == "1" ]]; then
            remote_frontier_dir="${REMOTE_BASE_DIR}/../kareus/schedules/${nemo_model_name}/${config_tag}/frontier"
        ...
        for freqs_path in "${RUN_FREQS_PATHS[@]}"; do
            ...
            suffix="${suffix%.py}"
            scheds_path="${SCHED_DIR}/scheds_pipeline_${suffix}.py"
            if [[ ! -f "${scheds_path}" ]]; then
                echo "    ERROR: Missing matching scheds file for ${freqs_path}: ${scheds_path}" >&2
                exit 1
            fi
            RUN_SCHEDS_PATHS+=("${scheds_path}")
    ...
    for run_idx in "${!RUN_FREQS_PATHS[@]}"; do
        ...
        scheds_path="${RUN_SCHEDS_PATHS[$run_idx]}"
        plan_tag="${RUN_PLAN_TAGS[$run_idx]}"

        if [[ "${FRONTIER}" == "true" ]]; then
            OUTPUT_DIR="${METHOD_DIR}/frontier/${plan_tag}"
            TRAIN_LOG="${LOG_DIR}/${nemo_model_name}_${config_tag}_kareus_${plan_tag}.log"
