...
# -*- coding: utf-8 -*-
"""
Scan a 'frontier' directory of runs, extract average time and energy from
Zeus monitor logs, and draw a scatter plot (Y: energy, X: time).
...
def plot_energy_time_multi(datasets: List[Tuple[str, List[Tuple[str, float, float]]]], out_path: Path) -> None:
    ...
    plt.title("Frontier: Energy vs Time")
...
def plot_effective_energy_time_multi(
    ...
    plt.title("Frontier: Effective Energy (E - P2P*Time) vs Time")
    plt.grid(True, linestyle=":", linewidth=0.6, alpha=0.7)
    plt.legend(loc="best")
...
def main() -> None:
    ...
    parser.add_argument(
        "--frontier_dir",
        type=str,
        default=str(Path(__file__).parent / "nemo_experiments/megatron_llama_3_2_1b/frontier"),
        help="Primary directory containing run subdirectories with Zeus monitor logs",
    )
    parser.add_argument(
        "--frontier_dir2",
        type=str,
        default=str((Path(__file__).parent.parent / "kareus/nemo_experiments/megatron_llama_3_2_1b/nanobatch_frontier").resolve()),
        help="Optional second directory to plot alongside the first",
    )
    parser.add_argument(
        "--frontier_dir3",
        type=str,
        default=str((Path(__file__).parent.parent / "kareus/nemo_experiments/megatron_llama_3_2_1b/kareus_frontier").resolve()),
    ...
    parser.add_argument(
        "--output",
        type=str,
        default=str(Path(__file__).parent / "nemo_experiments/megatron_llama_3_2_1b/frontier/energy_time.png"),
        help="Path to save the Energy vs Time figure",
    )
    parser.add_argument(
        "--output_effective",
        type=str,
        default=str(Path(__file__).parent / "nemo_experiments/megatron_llama_3_2_1b/frontier/energy_time_effective.png"),
    ...
    if frontier_dir2 is not None and frontier_dir2.exists():
        ...
        if len(pts2_sorted) > 0:
            datasets.append((args.label2, pts2_sorted))
        else:
    ...
    if frontier_dir3 is not None and frontier_dir3.exists():
        ...
        if len(pts3_sorted) > 0:
            datasets.append((args.label3, pts3_sorted))
        else:
