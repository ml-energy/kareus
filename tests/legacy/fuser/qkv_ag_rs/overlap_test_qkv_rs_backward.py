...
from kareus.megatron.core.extensions.fusers.qkv_fuser2 import QKVPartitionFuser as PartitionFuser
...
class AttentionFuserTest:
    ...
    def create_test_tensors(self):
        ...
        allreduce_key = torch.randn(
            local_seq_length, nano_batch_size, self.hidden_size,
            dtype=self.dtype, device=self.device
        )

        allreduce_value = torch.randn(
            local_seq_length, nano_batch_size, self.hidden_size,
            dtype=self.dtype, device=self.device
        )
        
        return hidden_states, bias, residual, rotary_pos_emb, attention_mask, allreduce_key, allreduce_value
    ...
    def create_gradient_tensors(self):
        ...
        reducescatter_key = torch.randn(
            self.seq_length, nano_batch_size, local_query_groups, self.head_dim,
            dtype=self.dtype, device=self.device
        )

        reducescatter_value = torch.randn(
            self.seq_length, nano_batch_size, local_query_groups, self.head_dim,
            dtype=self.dtype, device=self.device
        )

        global K_RS, V_RS
        K_RS[0] = reducescatter_key
        V_RS[0] = reducescatter_value
        
        return output_grad, key_grad, value_grad, residual_grad
    ...
    def test_backward_config(self, monitor, test_tensors, grad_tensors, attention_fuser, overlap_window, sm_configs):
        hidden_states, bias, residual, rotary_pos_emb, attention_mask, allreduce_key, allreduce_value = test_tensors
        query_grad, key_grad, value_grad, residual_grad = grad_tensors
        ...
        for i in range(10):
            ...
            if i == 0:
                query, key, value, residual_out = attention_fuser(
                    hidden_states=hidden_states,
                    bias=bias,
                    residual=residual,
                    rotary_pos_emb=rotary_pos_emb,
                    attention_mask=attention_mask,
                    comm_key=allreduce_key,
                    comm_value=allreduce_value,
