...
class LossProfiler:
    def __init__(self, args, rank: int, world_size: int):
        ...
        # Module that provides the CE implementation we want to profile
        self.loss_mod = LanguageModule(self.config).to(self.device)
        ...
        # labels shape: [b, s] global token ids
    ...
    def run(self):
        ...
        if self.rank == 0:
            ...
            with open(
                f"logs/{self.model_name}/cp{self.context_parallel_size}-tp{self.tensor_parallel_size}-bs{self.batch_size}-seq{self.seq_length}/{self.frequency}/loss_energy.csv",
                'w',
            ) as f:
        ...
        for freq_mhz in freq_values:
            if rank == 0:
                ...
                if visible is not None and len(visible.strip()) > 0:
                    ...
                    target_indices = None
                print(f'Profiling loss at frequency {freq_mhz} MHz')
            ...
            profiler = LossProfiler(args, rank, world_size)
            profiler.run()
...
if __name__ == '__main__':
    ...
    print('Running loss profiling (cross-entropy) over frequency sweep')
