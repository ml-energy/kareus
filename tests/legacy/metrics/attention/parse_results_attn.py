...
def compute_sm_idle_ratio(trace_df: pd.DataFrame) -> Optional[float]:
    ...
    idle_ratio = float((values < 30.0).sum()) / float(len(values))
    return idle_ratio
...
def collect_profile_data(results_dir: str) -> Tuple[Dict[ConfigKey, Dict[str, float]], Dict[ConfigKey, float], Dict[ConfigKey, float], Set[str]]:
    ...
    Returns:
        ...
        sm_compute_idle_ratio_map: config -> SM instructions < 20% ratio
...
def merge_and_write(
    ...
    try:
        ...
        if not xy.empty:
            ...
            plt.title("Energy vs SM idle ratio")
            plt.grid(True, alpha=0.3)
            out_path = os.path.join(plots_dir, "sm_idle_vs_energy.png")
        ...
        if not xy.empty:
            ...
            plt.ylabel("Total energy (J)")
            plt.title("Energy vs SM compute idle ratio")
            plt.grid(True, alpha=0.3)
            out_path = os.path.join(plots_dir, "sm_compute_idle_vs_energy.png")
            plt.savefig(out_path, bbox_inches="tight")
            plt.close()
            print(f"[\u2713] Figure written to {out_path}")
...
def main() -> None:
    ...
    output_csv = (
        args.output
        if args.output
        else os.path.join(args.log_dir, args.frequency, f"summary_attn_{args.frequency}.csv")
    )
