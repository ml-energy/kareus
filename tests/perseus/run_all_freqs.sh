...
FREQS_DIR=${1:-nemo_experiments/megatron_llama_3_2_1b/perseus_results}
# HOST=${2:-0.0.0.0}
PORT=${3:-7787}
...
SAMPLE_STRIDE=${SAMPLE_STRIDE:-200}
...
for f in "${FREQ_FILES[@]}"; do
  ...
  if ! python megatron_gpt_pretraining.py > "$train_log" 2>&1; then
