...
from kareus.megatron.core.extensions.attention_fuser_persistent import AttentionFuser
...
class AttentionFuserTest:
    ...
    def create_operations(self):
        ...
        linear_qkv_op.bias_fusable = True
        ...
        if self.tensor_parallel_size > 1:
            allreduce_comm_op_fwd = AllReduce(
                ...
                use_persistent_output=True,
                input_buffer=linear_proj_op.persistent_output,
                tensor_size=[self.seq_length, self.batch_size, self.hidden_size],
                device=self.device,
                dtype=self.dtype,
            )
            allreduce_comm_op_bwd = AllReduce(
                process_group=self.tp_group,
                async_op=True,  # Use async mode as set in the modified AllReduce
                backend="msccl",
                rank=self.rank,
                world_size=self.world_size,
                use_persistent_output=True,
                input_buffer=linear_qkv_op.persistent_output,
                tensor_size=[self.seq_length, self.batch_size, self.hidden_size],
                device=self.device,
                dtype=self.dtype,
            )
            ...
            return [
                ...
                attention_op,
                linear_proj_op,
                allreduce_comm_op_fwd,
                allreduce_comm_op_bwd,
            ]
    ...
    def run_overlap_test(self):
        ...
        allreduce_comm_op_fwd = operations[7]
        allreduce_comm_op_bwd = operations[8]
        
        allreduce_inputs = comp_ops[-1].persistent_output
        test_tensors = test_tensors + (allreduce_inputs,)

        attention_fuser = AttentionFuser(
            ops=comp_ops,
            allreduce_comm_op_fwd=allreduce_comm_op_fwd,
            allreduce_comm_op_bwd=allreduce_comm_op_bwd,
            fuse_ops=True
        )
