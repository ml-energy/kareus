...
class AttentionFuserTest:
    ...
    def test_config(self, monitor, test_tensors, grad_tensors, attention_fuser, overlap_window, sm_configs):
        ...
        if self.rank == 0:
            with open(f"logs/a_rs/{self.model_name}/cp{self.context_parallel_size}-tp{self.tensor_parallel_size}-bs{self.batch_size}-seq{self.seq_length}/{self.frequency}/backward_results.csv", "a") as f:
    ...
    def run_overlap_test(self):
        ...
        if self.rank == 0:
            ...
            os.makedirs(f"logs/a_rs/{self.model_name}/cp{self.context_parallel_size}-tp{self.tensor_parallel_size}-bs{self.batch_size}-seq{self.seq_length}/{self.frequency}", exist_ok=True)
            with open(f"logs/a_rs/{self.model_name}/cp{self.context_parallel_size}-tp{self.tensor_parallel_size}-bs{self.batch_size}-seq{self.seq_length}/{self.frequency}/backward_results.csv", "w") as f:
