...
# -*- coding: utf-8 -*-
"""
Profile backward (autograd) time and energy for the MLP fuser only.
...
class MLPFuserBackwardProfiler:
    """
    Build and profile MLP fuser forward + backward.
    ...
    def __init__(self, args: argparse.Namespace, rank: int, world_size: int) -> None:
        ...
        # Build MLP tensors (requires grad)
        ...
        # Backward gradient tensors for MLP outputs
    ...
    def _run_one_step_backward(self):
        ...
        _ = torch.autograd.grad(
            ...
            inputs=[self.mlp_hidden_states, self.mlp_residual, self.mlp_allreduce_inputs],
    ...
    def profile(self, monitor=None):
        if self.rank == 0:
            ...
            csv_path = os.path.join(logs_dir, "mlp_backward_energy.csv")
        ...
        if self.rank == 0:
            print(f"[MLPBackward] Per-step duration ~ {duration*1000:.3f} ms -> {iterations} iterations")
        ...
        if self.rank == 0:
            ...
            print(f"[MLPBackward] avg_time={avg_time:.6f}s, avg_total_energy={avg_total_energy:.4f}J")
...
def _freq_sweep_worker(rank: int, world_size: int, args: argparse.Namespace, master_port: int, freq_values):
    ...
    try:
        ...
        profiler = MLPFuserBackwardProfiler(args, rank, world_size)
        ...
        for freq_mhz in freq_values:
            if rank == 0:
                ...
                if visible is not None and len(visible.strip()) > 0:
                    ...
                    target_indices = None
                print(f"[MLPFuserBackward] profiling at frequency {freq_mhz} MHz")
...
def main():
    ...
    print("Profiling MLP Fuser Backward over frequency sweep")
