...
# -*- coding: utf-8 -*-
"""
Profile energy and time of a Transformer layer by composing attention and MLP fusers.
...
class TransformerLayerFuserProfiler:
    """
    Compose attention and MLP fusers to emulate a full transformer layer forward.
    ...
    def _run_one_step(self):
        ...
        return mlp_out, mlp_out_bias, mlp_out_residual

    def profile(self, monitor=None):
        if self.rank == 0:
            logs_dir = self._logs_dir()
            csv_path = os.path.join(logs_dir, "transformer_layer_energy.csv")
        ...
        if self.rank == 0:
            print(f"[TransformerLayer] Per-step duration ~ {duration*1000:.3f} ms -> {iterations} iterations")
        ...
        if self.rank == 0:
            ...
            print(f"[TransformerLayer] avg_time={avg_time:.6f}s, avg_total_energy={avg_total_energy:.4f}J")
...
def _freq_sweep_worker(rank: int, world_size: int, args: argparse.Namespace, master_port: int, freq_values):
    ...
    try:
        ...
        profiler = TransformerLayerFuserProfiler(args, rank, world_size)
        ...
        for freq_mhz in freq_values:
            if rank == 0:
                ...
                if visible is not None and len(visible.strip()) > 0:
                    ...
                    target_indices = None
                print(f"[TransformerLayerFuser] profiling at frequency {freq_mhz} MHz")
...
def main():
    ...
    print("Profiling Transformer Layer Fuser (attention + MLP) over frequency sweep")
