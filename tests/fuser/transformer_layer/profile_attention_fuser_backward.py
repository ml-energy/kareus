...
# -*- coding: utf-8 -*-
"""
Profile backward (autograd) time and energy for the Attention fuser only.
...
# Ops used to construct attention fuser
...
class AttentionFuserBackwardProfiler:
    """
    Build and profile attention fuser forward + backward.
    ...
    def __init__(self, args: argparse.Namespace, rank: int, world_size: int) -> None:
        ...
        # Build attention tensors (requires grad)
        ...
        # Backward gradient tensors for attention outputs
        ...
        self.allreduce_output_grad = torch.randn(
            self.seq_length, self.batch_size, self.hidden_size,
            dtype=self.dtype, device=self.device
        )

        # Cached forward outputs for reuse across backward steps
        self.attn_out = None
        self.attn_out_residual = None
        self.attn_allreduce_out = None
    ...
    def _run_one_step_backward(self):
        if self.attn_out is None:
            attn_out, attn_out_bias, attn_out_residual, attn_allreduce_out = self.attn_fuser(
            ...
            self.attn_out_residual = attn_out_residual
            self.attn_allreduce_out = attn_allreduce_out
        _ = torch.autograd.grad(
            outputs=[self.attn_out, self.attn_out_residual, self.attn_allreduce_out],
            inputs=[self.attn_hidden_states, self.attn_residual, self.attn_allreduce_inputs],
            grad_outputs=[self.output_grad, self.residual_grad, self.allreduce_output_grad],
            retain_graph=True,
            allow_unused=True,
            create_graph=False,
        )
    ...
    def profile(self, monitor=None):
        if self.rank == 0:
            ...
            csv_path = os.path.join(logs_dir, "attention_backward_energy.csv")
        ...
        if self.rank == 0:
            print(f"[AttentionBackward] Per-step duration ~ {duration*1000:.3f} ms -> {iterations} iterations")
        ...
        if self.rank == 0:
            ...
            print(f"[AttentionBackward] avg_time={avg_time:.6f}s, avg_total_energy={avg_total_energy:.4f}J")
...
def _freq_sweep_worker(rank: int, world_size: int, args: argparse.Namespace, master_port: int, freq_values):
    ...
    try:
        ...
        profiler = AttentionFuserBackwardProfiler(args, rank, world_size)
        ...
        for freq_mhz in freq_values:
            if rank == 0:
                ...
                if visible is not None and len(visible.strip()) > 0:
                    ...
                    target_indices = None
                print(f"[AttentionFuserBackward] profiling at frequency {freq_mhz} MHz")
...
def main():
    ...
    print("Profiling Attention Fuser Backward over frequency sweep")
