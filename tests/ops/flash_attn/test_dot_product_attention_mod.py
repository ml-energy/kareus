...
"""Test DotProductAttention implementation and save results."""
import sys
...
def test_dot_product_attention_mod():
    """Test DotProductAttention and save results."""
    
    print("🧪 Testing DotProductAttention (Traditional Module)")
    print("=" * 70)
    
    # Test parameters (must match the op test)
    batch_size = 2
    seq_len = 128
    num_heads = 8
    head_dim = 64
    ...
    # Create test tensors with fixed seed for reproducibility (same as op test)
    torch.manual_seed(42)
    torch.cuda.manual_seed(42) if torch.cuda.is_available() else None
    
    # Create input tensors (matching op test format: sbhd)
    ...
    try:
        ...
        print("\n📦 Importing DotProductAttention...")
        from kareus.transformer_engine.pytorch.attention.dot_product_attention.dot_product_attention import DotProductAttention
        print("✅ Successfully imported DotProductAttention")
        
        # Test configurations (must match the op test)
        test_configs = [
        ...
        for config in test_configs:
            ...
            try:
                ...
                attention_mod = DotProductAttention(
                    num_attention_heads=num_heads,
                    kv_channels=head_dim,
                    qkv_format="sbhd",
                    **config
                )
                attention_mod.eval()  # Set to eval mode for consistent behavior
                
                # Forward pass
                print("   🔄 Running forward pass...")
                output = attention_mod(q_test, k_test, v_test)
                ...
                # Backward pass using autograd (matching op test)
        ...
        save_data = {
            ...
            'test_config': {
                ...
                'device': str(device),
                'requires_grad': True,
                'implementation': 'DotProductAttention'
            }
        }
        
        output_file = "dot_product_attention_mod_results.pt"
        ...
        if successful == len(test_configs):
            print("   🎉 All DotProductAttention tests PASSED!")
            return True
        else:
            print("   ⚠️  Some DotProductAttention tests FAILED!")
            return False
            
    except ImportError as e:
        print(f"❌ Failed to import DotProductAttention: {e}")
        traceback.print_exc()
        return False
    except Exception as e:
...
if __name__ == "__main__":
    print("Running DotProductAttention test...")
    success = test_dot_product_attention_mod()
