...
def read_prepost_profile(
    ...
    for frequency in freqs:
        ...
        def safe_read(p):
            # try:
            return _read_time_energy_csv(p)
            # except Exception as e:
            #     print(f"  Warning: could not read {p}: {e}. Using zeros.")
            #     return 0.0, 0.0

        emb = safe_read(emb_path)
        out = safe_read(out_path)
        loss = safe_read(loss_path)
        results.append({
        ...
        for attn_config, attn_result in attention_bwd_result:
            for mlp_config, mlp_result in mlp_bwd_result:
                ...
                for stage in range(pipeline_parallel_size):
                    ...
                    if stage == 0:
                        bwd_time += prepost_profiling_results[freq_idx]["forward-embedding"][0]
                        bwd_energy += prepost_profiling_results[freq_idx]["forward-embedding"][1]
                    elif stage == pipeline_parallel_size - 1:
                        bwd_time += prepost_profiling_results[freq_idx]["forward-output"][0] * 2
                        bwd_energy += prepost_profiling_results[freq_idx]["forward-output"][1] * 2
