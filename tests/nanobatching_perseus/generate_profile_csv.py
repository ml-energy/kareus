...
Scans a multi-node profiling directory (node0/, node1/, ...) where each node
contains per-frequency subdirectories with timer and energy CSVs.  For every
GPU frequency, it:
...
def main(
    ...
    # --- discover frequency directories from the first node ---
    ...
    # --- determine global ranks from the first frequency ---
    ...
    for freq_name in freq_names:
        ...
        # Read instruction timing results for this frequency
        ...
        # Time: max across TP ranks; Energy: sum across TP ranks
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate profile.csv from multi-node profiling data."
    )
    parser.add_argument(
        "--profile_dir",
        required=True,
        help="Directory containing profiling results (with node0/, node1/ subdirs).",
    )
    parser.add_argument("--num_microbatches", default=8, type=int)
    parser.add_argument("--num_prof_iters", default=20, type=int)
    parser.add_argument("--warmup_iters", default=10, type=int)
    parser.add_argument(
        "--num_ranks_per_stage", default=8, type=int,
        help="Number of ranks per pipeline stage (CP * TP).",
    )
