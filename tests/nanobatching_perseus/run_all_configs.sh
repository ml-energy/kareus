#!/usr/bin/env bash
# Automated Nanobatch+Perseus test: profiling → CSV → optimization → PFO + training
...
echo "===== Nanobatch+Perseus 16-GPU Automated Tests (PP=${PP}, #microbatches=${NUM_MICROBATCHES}) ====="
...
for i in "${!CONFIGS[@]}"; do
    ...
    if [[ "${NODE_RANK}" == "0" ]]; then
        ...
        if [[ -z "${freqs_path}" ]]; then
            ...
            echo "    Skipping Nanobatch+Perseus training for this config." >&2
            continue
        fi
