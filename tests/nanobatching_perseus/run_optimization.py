...
DEFAULT_P2P_POWER = 85.0  # A100 p2p idle power in Watts
DEFAULT_PLOT_INTERVAL = 100
DEFAULT_UNIT_TIME = 0.001
DEFAULT_NOISE_FACTOR = 0.95
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Perseus pipeline frequency optimisation."
    )
    parser.add_argument(
        "--inst_profile", required=True,
        help="Path to profile.csv from generate_profile_csv.py.",
    )
    parser.add_argument(
        "--output_dir", required=True,
        help="Directory to write freqs_pipeline_*.py solutions and plots.",
    )
    parser.add_argument("--num_mbs", default=DEFAULT_NUM_MBS, type=int,
                        help="Number of microbatches.")
    parser.add_argument("--num_stages", default=DEFAULT_NUM_STAGES, type=int,
                        help="Number of pipeline stages.")
    parser.add_argument("--p2p_power", default=DEFAULT_P2P_POWER, type=float,
                        help="GPU power (W) while blocking on P2P communication.")
    parser.add_argument("--plot_interval", default=DEFAULT_PLOT_INTERVAL, type=int,
                        help="Draw pipeline state every N iterations.")
    parser.add_argument("--unit_time", default=DEFAULT_UNIT_TIME, type=float,
                        help="Unit of time reduction per solver iteration (seconds).")
    parser.add_argument("--noise_factor", default=DEFAULT_NOISE_FACTOR, type=float,
                        help="Noise factor for soft Pareto frontier filtering.")
