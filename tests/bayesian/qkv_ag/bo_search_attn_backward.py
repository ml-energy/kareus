...
Mirrors the shared BO pipeline and reuses bo_utils.
"""
...
class PartitionTestConfig:
    """
    Config holder for bo_utils compatibility (QKV-AG backward).
    """
...
class PartitionTestRunner:
    ...
    def test_config(self, overlap_window, sm_configs):
        ...
        torch.autograd.backward(
            tensors=[self.query, self.key, self.value, self.residual_out],
            grad_tensors=[self.query_grad, self.key_grad, self.value_grad, self.residual_grad],
            retain_graph=True,
        )
...
def main() -> None:
    ...
    print("===============================================")
    print("Bayesian Optimization for QKV-AG Attention Fuser (backward; real measurements)")
    print("===============================================")
    ...
    for ib in range(int(start_batch_idx), int(args.batches)):
        ...
        print("Evaluating selected candidates on hardware (QKV-AG backward)...")
