...
def measure_on_hardware(
    ...
    # Randomize master port to avoid collisions
    master_port = random.randint(12000, 65000)

    # Create a shared manager and dictionary for results
    manager = mp.Manager()
    ...
    # Get results from shared dictionary
    energy_j = float(shared_results["energy_j"])
    time_s = float(shared_results["time_s"])
    # else:
    #     # Fallback to file reading if shared results are not available
    #     if os.path.exists(eval_log_path):
    #         with open(eval_log_path, "r") as f:
    #             for line in f:
    #                 rec = json.loads(line)
    #                 if rec.get("uid") == run_uid:
    #                     energy_j = float(rec.get("energy_j"))
    #                     time_s = float(rec.get("time_s"))
    #                     break
...
def measure_from_profile_results(
    ...
    # Construct the path to the CSV file based on the configuration
    # Format: logs_base_dir/tp{world_size}-bs{batch_size}-seq{seq_len}/{freq_mhz}/energy_results.csv
    ...
    # The CSV columns are: overlap_start,overlap_end,comm_sm_number,comm_block_size,0:time (s),0:total energy (J),...
    matching_rows = df[
    ...
    # Get the first matching row (there should typically be only one)
    row = matching_rows.iloc[0]
    
    # Extract energy and time values
    # The CSV format uses "0:time (s)" and "0:total energy (J)" as column names
...
def expected_hypervolume_improvement(
    ...
    if normalization_bounds is not None:
        ...
        # Normalize current Pareto front
        ...
        # Calculate hypervolume with normalized values
