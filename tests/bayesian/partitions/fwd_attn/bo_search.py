#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Bayesian optimization for forward attention partition (TP, ALL_REDUCE)."""
...
SEARCH_SPACE = SearchSpace(
    overlap_windows=[("norm_0", "none"), ("linear_0", "none"), ("rope_0", "none"), ("attn_0", "none")],
...
BO_CONFIG = BOSearchConfig(
    banner="Forward Attention Partition (TP, ALL_REDUCE)",
    logs_dir_fn=lambda args: (
        f"logs/{args.model_name}/cp{args.context_parallel_size}"
        f"-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}/fwd_attn"
    ),
...
class PartitionTestRunner:
    """BO-compatible wrapper around PartitionTest."""
