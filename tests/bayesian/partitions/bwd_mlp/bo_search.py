#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Bayesian optimization for backward MLP partition (TP, ALL_REDUCE)."""
...
SEARCH_SPACE = SearchSpace(
    overlap_windows=[("norm_0", "none"), ("linear_0", "none"), ("activation_0", "none")],
...
BO_CONFIG = BOSearchConfig(
    banner="Backward MLP Partition (TP, ALL_REDUCE)",
    logs_dir_fn=lambda args: (
        f"logs/{args.model_name}/cp{args.context_parallel_size}"
        f"-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}/bwd_mlp"
    ),
