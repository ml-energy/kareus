#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Bayesian optimization for Backward A-RS Partition (CP, REDUCE_SCATTER_KV)."""
...
BO_CONFIG = BOSearchConfig(
    banner="Backward A-RS Partition (CP+TP, REDUCE_SCATTER_KV)",
    logs_dir_fn=lambda args: (
        f"logs/{args.model_name}/cp{args.context_parallel_size}"
        f"-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}/bwd_a_rs"
    ),
