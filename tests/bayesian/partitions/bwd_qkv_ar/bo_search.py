#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Bayesian optimization for Backward QKV-AR Partition (CP, ALL_REDUCE)."""
...
SEARCH_SPACE = SearchSpace(
    overlap_windows=[("rope_0", "none"), ("linear_0", "none")],
...
BO_CONFIG = BOSearchConfig(
    banner="Backward QKV-AR Partition (CP+TP, ALL_REDUCE)",
    logs_dir_fn=lambda args: (
        f"logs/{args.model_name}/cp{args.context_parallel_size}"
        f"-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}/bwd_qkv_ar"
    ),
