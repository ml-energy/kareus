#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Bayesian optimization for Forward QKV-AR Partition (CP, ALL_REDUCE)."""
...
BO_CONFIG = BOSearchConfig(
    banner="Forward QKV-AR Partition (CP, ALL_REDUCE)",
    logs_dir_fn=lambda args: (
        f"logs/{args.model_name}/cp{args.context_parallel_size}"
        f"-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}/fwd_qkv_ar"
    ),
