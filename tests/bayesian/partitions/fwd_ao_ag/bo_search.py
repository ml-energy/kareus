#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Bayesian optimization for Forward AO-AG Partition (CP, ALL_GATHER_KV)."""
...
SEARCH_SPACE = SearchSpace(
    overlap_windows=[("attn_0", "none"), ("linear_0", "none")],
...
BO_CONFIG = BOSearchConfig(
    banner="Forward AO-AG Partition (CP, ALL_GATHER_KV)",
    logs_dir_fn=lambda args: (
        f"logs/{args.model_name}/cp{args.context_parallel_size}"
        f"-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}/fwd_ao_ag"
    ),
