...
Mirrors the attention forward optimizer but targets the MLP fuser.
"""
...
class PartitionTestConfig:
    """
    Lightweight configuration holder for bo_utils compatibility (forward MLP).
...
def main() -> None:
    ...
    # CSV logging for per-batch timings (forward MLP)
    timing_csv_path = os.path.join(partition_test.logs_dir, "bo_timing_fwd.csv")
    ...
    for ib in range(int(start_batch_idx), int(args.batches)):
        ...
        print("Evaluating selected candidates on hardware (forward MLP)...")
