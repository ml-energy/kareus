...
def _dist_eval_worker(
    ...
    shared_results: dict,
    eval_log_path: str,
    selection_flags: Dict[str, bool] | None = None,
...
def _save_iteration_plots(
    ...
    base_logs_dir = f"logs/tp{args.world_size}-bs{args.batch_size}-seq{args.seq_len}/forward/figures"
    ...
    try:
        ...
        print(f"Warning: failed to save MLP effective plot for iter {ib+1}: {_exc}")
    ...
    try:
        ...
        print(f"Warning: failed to save MLP real plot for iter {ib+1}: {_exc}")
# -----------------------------
...
def try_load_initial_from_cache(
    ...
    try:
        ...
        # If cached unique points are fewer than requested, generate the rest and evaluate
        ...
        if missing > 0:
            ...
            # Filter out already included cached configs
