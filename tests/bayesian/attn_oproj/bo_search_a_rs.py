...
# -*- coding: utf-8 -*-
"""
Bayesian optimization for Attention OProj fuser (A-RS, backward) overlap-window
and communication configs using real hardware measurements (time and energy).

Mirrors the A-AG backward optimizer but targets the A-RS harness.
"""
...
from overlap_test_a_rs import AttentionFuserTest  # noqa: E402
...
# Editable (backward-specific windows for A-RS)
OVERLAP_WINDOWS = [
    (0, 0),
]

# A-RS test sweeps SM 1..20
...
class PartitionTestConfig:
    """
    Lightweight configuration holder for bo_utils compatibility (A-RS backward).
...
class PartitionTestRunner:
    """
    Initializes tensors and A-RS fuser for backward pass.
    ...
    def __init__(self, args: argparse.Namespace, rank: int, world_size: int) -> None:
        ...
        self.attention_fuser = PartitionFuser(
            ...
            comm_ops_bwd=[comm_op, None, None, None],
            fuse_ops=False,
            profile_a_rs=True,
        )
    ...
    def test_config(self, overlap_window, sm_configs):
        if self.func_ctx is None:
            self.out_1, self.out_2, self.bias_1, self.bias_2, self.func_ctx = self.attention_fuser(
                ...
                comm_overlap_window_a_rs=overlap_window,
                comm_sm_configs_a_rs=sm_configs,
            )
...
def main() -> None:
    ...
    print("===============================================")
    print("Bayesian Optimization for Attention OProj Fuser (A-RS backward; real measurements)")
    print("===============================================")
    ...
    for ib in range(int(start_batch_idx), int(args.batches)):
        ...
        print("Evaluating selected candidates on hardware (A-RS backward)...")
