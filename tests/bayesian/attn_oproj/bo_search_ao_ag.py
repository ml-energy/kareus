...
Mirrors the AO-AR optimizer but targets the AO-AG harness.
"""
...
# -----------------------------
# Forward configuration and runner
# -----------------------------

class PartitionTestConfig:
    """
    Lightweight configuration holder for bo_utils compatibility (AO-AG forward).
    Used only in parent process.
    """
    def __init__(self, args: argparse.Namespace):
        self.args = args
        # For AO-AG, world_size == context_parallel_size
        logs_dir = f"logs/ao_ag/{args.model_name}/cp{args.world_size}-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}"
...
class PartitionTestRunner:
    ...
    def __init__(self, args: argparse.Namespace, rank: int, world_size: int) -> None:
        ...
        # For AO-AG, the distributed group is the context-parallel group
...
def main() -> None:
    ...
    print("===============================================")
    print("Bayesian Optimization for Attention OProj Fuser (AO-AG forward; real measurements)")
    print("===============================================")
    ...
    for ib in range(int(start_batch_idx), int(args.batches)):
        ...
        print("Evaluating selected candidates on hardware (AO-AG forward)...")
        sel_flags_list: List[Dict[str, bool]] = []
        sel_preds_list: List[Dict[str, float]] = []
        for i, vec in enumerate(selected):
            sel_i = final_idx[i]
            flags = {
                "selected_exploit_eff": bool(sel_i in exploit_eff_idx),
                "selected_exploit_real": bool(sel_i in exploit_real_idx),
                "selected_time": bool(sel_i in time_idx),
                "selected_explore": bool(sel_i in explore_idx),
            }
        ...
        from bo_utils import save_iteration_plots  # local import to avoid circulars in tooling
