...
# -*- coding: utf-8 -*-
"""
Bayesian optimization for Attention OProj fuser (A-AG, backward) overlap-window
and communication configs using real hardware measurements (time and energy).

Mirrors the QKV backward optimizer but targets the A-AG fuser.
"""
...
from overlap_test_a_ag import AttentionFuserTest  # noqa: E402
...
# Editable (backward-specific windows for A-AG)
OVERLAP_WINDOWS = [
    (0, 0),
]

# A-AG test sweeps SM 1..20
...
class PartitionTestConfig:
    """
    Lightweight configuration holder for bo_utils compatibility (A-AG backward).
    Used only in parent process.
    """
    def __init__(self, args: argparse.Namespace):
        self.args = args
        logs_dir = f"logs/a_ag/{args.model_name}/cp{args.context_parallel_size}-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}"
...
class PartitionTestRunner:
    """
    Initializes tensors and A-AG fuser for backward pass.
    ...
    @property
    def tp_group(self):
        # For API compatibility; bo_utils uses .group
        return self.group
...
def main() -> None:
    ...
    print("===============================================")
    print("Bayesian Optimization for Attention OProj Fuser (A-AG backward; real measurements)")
    print("===============================================")
    ...
    for ib in range(int(start_batch_idx), int(args.batches)):
        ...
        print("Evaluating selected candidates on hardware (A-AG backward)...")
