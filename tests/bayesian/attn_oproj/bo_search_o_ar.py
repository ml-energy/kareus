...
# -*- coding: utf-8 -*-
"""
Bayesian optimization for Attention OProj fuser (O-AR, backward) overlap-window
and communication configs using real hardware measurements (time and energy).

Mirrors the A-AG backward optimizer but targets the O-AR harness.
"""
...
from overlap_test_o_ar import AttentionFuserTest  # noqa: E402
...
# Editable (backward-specific windows for O-AR)
OVERLAP_WINDOWS = [
    (0, 1),
]
...
class PartitionTestConfig:
    """
    Lightweight configuration holder for bo_utils compatibility (O-AR backward).
    Used only in parent process.
    """
    def __init__(self, args: argparse.Namespace):
        self.args = args
        # For O-AR, world_size == tensor_parallel_size
        logs_dir = f"logs/o_ar/{args.model_name}/cp{args.context_parallel_size}-tp{args.world_size}-bs{args.batch_size}-seq{args.seq_len}"
...
class PartitionTestRunner:
    """
    Initializes tensors and O-AR fuser for backward pass.
    ...
    def __init__(self, args: argparse.Namespace, rank: int, world_size: int) -> None:
        ...
        self.attention_fuser = PartitionFuser(
            ...
            comm_ops_bwd=[None, None, None, comm_op],
            fuse_ops=False,
            profile_o_ar=True,
        )
        ...
        # O-AR uses tensor-parallel group (tp_group)
    ...
    def test_config(self, overlap_window, sm_configs):
        if self.func_ctx is None:
            self.out_1, self.out_2, self.bias_1, self.bias_2, self.func_ctx = self.attention_fuser(
                ...
                comm_overlap_window_o_ar=overlap_window,
                comm_sm_configs_o_ar=sm_configs,
            )
...
def main() -> None:
    ...
    print("===============================================")
    print("Bayesian Optimization for Attention OProj Fuser (O-AR backward; real measurements)")
    print("===============================================")
    ...
    for ib in range(int(start_batch_idx), int(args.batches)):
        ...
        print("Evaluating selected candidates on hardware (O-AR backward)...")
        ...
        from bo_utils import save_iteration_plots  # local import for plotting
