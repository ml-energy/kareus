...
Mirrors the A-AG backward optimizer but targets the O-AG harness.
"""
...
# Editable (backward-specific windows for O-AG)
OVERLAP_WINDOWS = [
    (0, 1),
]

# O-AG test sweeps SM 1..20
...
class PartitionTestConfig:
    """
    Lightweight configuration holder for bo_utils compatibility (O-AG backward).
    Used only in parent process.
    """
    def __init__(self, args: argparse.Namespace):
        self.args = args
        logs_dir = f"logs/o_ag/{args.model_name}/cp{args.context_parallel_size}-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}"
...
def main() -> None:
    ...
    print("===============================================")
    print("Bayesian Optimization for Attention OProj Fuser (O-AG backward; real measurements)")
    print("===============================================")
    ...
    for ib in range(int(start_batch_idx), int(args.batches)):
        ...
        print("Evaluating selected candidates on hardware (O-AG backward)...")
