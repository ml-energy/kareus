...
def _dist_eval_worker(
    ...
    query_grad, key_grad, value_grad, residual_grad, allreduce_input_grad = (
        test.create_gradient_tensors()
    )
    ...
    for i in range(10):
        if i == 0:
            query, key, value, residual_out, allreduce_output = attention_fuser(
                ...
                comm_sm_configs=(30, 1024),
                comm_overlap_window_backward=overlap_window,
                comm_sm_configs_backward=(sm_num, block_size),
            )
        if i == 2:
...
def _save_iteration_plots(
    ...
    base_logs_dir = f"logs/{args.model_name}/cp{args.context_parallel_size}-tp{args.tensor_parallel_size}-bs{args.batch_size}-seq{args.seq_len}/backward/figures"
