...
Mirrors the attention backward optimizer but targets the QKV fuser.
"""
...
from kareus.megatron.core.extensions.qkv_fuser import QKVPartitionFuser as PartitionFuser  # noqa: E402
...
class PartitionTestConfig:
    """
    Lightweight configuration holder for bo_utils compatibility (QKV backward).
    Used only in parent process.
    """
    def __init__(self, args: argparse.Namespace):
        self.args = args
        logs_dir = f"logs/{args.model_name}/cp{args.context_parallel_size}-tp{args.world_size}-bs{args.batch_size}-seq{args.seq_len}/backward"
...
class PartitionTestRunner:
    ...
    def test_config(self, overlap_window, sm_configs):
        ...
        torch.autograd.backward(
            tensors=[self.query, self.key, self.value, self.residual_out, self.allreduce_output],
            grad_tensors=[self.query_grad, self.key_grad, self.value_grad, self.residual_grad, self.allreduce_input_grad],
            retain_graph=True,
        )
...
def main() -> None:
    ...
    print("===============================================")
    print("Bayesian Optimization for QKV Attention Fuser (backward; real measurements)")
    print("===============================================")
    ...
    for ib in range(int(start_batch_idx), int(args.batches)):
        ...
        print("Evaluating selected candidates on hardware (QKV backward)...")
