...
# -*- coding: utf-8 -*-
"""
Bayesian optimization for QKV attention fuser (forward) overlap-window and communication configs
using real hardware measurements (time and energy) per candidate.

Mirrors the attention forward optimizer but targets the QKV fuser.
"""
...
from overlap_test_qkv_ar import AttentionFuserTest  # noqa: E402
...
class PartitionTestConfig:
    """
    Lightweight configuration holder for bo_utils compatibility (QKV forward).
...
class PartitionTestRunner:
    """
    Initializes tensors and QKV fuser for forward pass.
...
def main() -> None:
    ...
    print("===============================================")
    print("Bayesian Optimization for QKV Attention Fuser (forward; real measurements)")
    print("===============================================")
    ...
    for ib in range(int(start_batch_idx), int(args.batches)):
        ...
        print("Evaluating selected candidates on hardware (QKV forward)...")
