...
def _compute_group_durations(df: pd.DataFrame, cols: Dict[str, Optional[str]], gpu_type: str) -> List[Dict[str, object]]:
...
def derive_real_overlap_info_from_timeline(
    ...
    try:
        ...
        group_durations = _compute_group_durations(df, cols, gpu_type)
...
def plot_energy_vs_sm(
    ...
    if "legend_start_label" in filtered.columns:
        ...
        desired_order = ["No overlap", "RMSNorm", "Linear1", "SiLU", "Linear2"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot energy vs SM number for MLP logs")
    ...
    parser.add_argument("--launch_time_only", action="store_true", help="Consider launch time only: use nominal scopes and restrict to (0,6),(2,6),(4,6),(5,6); label by first group name")
    ...
    # df = df[pd.to_numeric(df["comm_sm_number"], errors="coerce") >= 2]
    df = df[pd.to_numeric(df["comm_block_size"], errors="coerce") == 1024]

    if args.launch_time_only:
        # Same selection logic as heatmap; do not compute real overlap
        ...
        mask_targets = (start_int.isin(valid_starts) & ((end_int == 6) | (end_int == 5))) | ((start_int == -1) & (end_int == -1))
    ...
    output_path = args.output if args.output else os.path.join(logs_dir, f"mlp_energy_vs_sm_{args.frequency}.png")
    ...
    if not args.no_heatmap:
        heatmap_out = os.path.join(logs_dir, "energy_vs_sm_heatmap.png")
        try:
