#!/usr/bin/env python3
"""
Process nsys profile results for MLP: export to SQLite and draw timelines.
Usage:
...
# python process_profile_results.py -i profile_result/event/tp8-bs8-seq4096
...
def draw_timeline_from_kernel_data(kernel_df, output_png, overlap_window, title=None, gpu_type="ampere", phase="forward"):
    ...
    Args:
        phase: "forward" or "backward" - determines kernel order
               forward: pre, fc1, act, fc2
               backward: pre, fc2, act, fc1
    """
    ...
    # Define kernel groups for timeline based on GPU type
    ...
    # forward order: pre, fc1, act, fc2
    # backward order: pre, fc2, act, fc1
    if phase == "backward":
        groups = [
            ...
            ("act", (4, 4), ['triton_poi_fused_mul_silu_backward', 'triton_poi_fused']),
            ("fc1", (5, 6), ['ampere_bf16_s16816gemm_bf16_128x128_ldg8_f2f_stages_32x5_nn', 'ampere_bf16_s16816gemm_bf16_256x128_ldg8_f2f_stages_64x3_nt']),
        ]
    else:  # forward (default)
        groups = [
            # ("pre", (0, 1), ['triton_poi_fused_add_0', 'triton_poi_fused_native_dropout_1', 'rmsnorm_fwd_general_kernel', 'rmsnorm_fwd_tuned_kernel']),
...
def process_single_report(report, output_base_dir, gpu_type="ampere", force_sqlite=False, skip_timeline=False):
    ...
    if not skip_timeline:
        ...
        title = f"MLP {report['phase']} | ({report['overlap_start']},{report['overlap_end']}) | SM {report['sm_num']}, Block {report['block_size']} | Freq {report['frequency']}"
...
def main():
    parser = argparse.ArgumentParser(description="Process nsys profile results for MLP: export to SQLite and draw timelines")
