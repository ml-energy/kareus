...
def process_sqlite(metrics_file):
    ...
    # df = df[df["GR Active [Throughput %]"] > 0]
...
if __name__ == "__main__":
    ...
    # if results:
    #     results_df = pd.DataFrame(results)
    #     results_df.to_csv(f"results/{frequency}/all_results_mlp_{frequency}.csv", index=False)
    #     print(f"[✓] Aggregated results written to results/{frequency}/all_results_mlp_{frequency}.csv")
