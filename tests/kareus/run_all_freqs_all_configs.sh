...
###############################################
# Run run_all_freqs.sh for all configs        #
###############################################
#
# Usage:
#   ./run_all_freqs_all_configs.sh <node_rank> [host] [port]
# where <node_rank> is 0 or 1.
#
# This script iterates over a set of Kareus configs for LLaMA 3.2 3B and,
# for each config, calls run_all_freqs.sh to run all freqs frontier plans:
...
for cfg in "${configs[@]}"; do
  results_dir="${SCRIPT_DIR}/${model_name}/${cfg}/nanobatch_frontier"
  ...
  echo "============================================"
  echo "Running all freqs for config: ${cfg}"
  echo "Using results_dir: ${results_dir}"
  echo "============================================"

  CONFIG="${cfg}" MODEL_NAME="${model_name}" \
    "${SCRIPT_DIR}/run_all_freqs.sh" "${NODE_RANK}" "${results_dir}" "${HOST}" "${PORT}"
done

echo "Completed run_all_freqs.sh for all configs on node${NODE_RANK}."
