"""Pipeline frequency optimization for Kareus.

Unified script supporting both non-CP and CP modes.
Auto-detects CP mode from CSV header (presence of 'fwd_qkv_ar' column).
"""
...
def _s(row: dict, key: str) -> str:
    """Coerce possibly-NaN value to empty string."""
    val = row.get(key, "")
    if val is None:
        return ""
    s = str(val)
...
def _build_options_non_cp(inst_df, instruction, stage_id, args):
    """Build ExecutionOption list for non-CP mode."""
    inst_name = instruction.__name__
    _df = inst_df.query(f"stage == {stage_id} and instruction == '{inst_name.lower()}'")
    options = []
        ...
        if instruction is Backward and args.use_activation_checkpointing:
            options.append(
                ExecutionOption[tuple[int, str, str, str, str]](
                    ...
                    knob=(
                        int(row["frequency"]),
                        _s(row, "recompute_attention_configs"),
                        _s(row, "recompute_mlp_configs"),
                        _s(row, "attention_configs"),
                        _s(row, "mlp_configs"),
                    ),
                )
            )
...
def _build_options_cp(inst_df, instruction, stage_id, args):
    """Build ExecutionOption list for CP mode."""
    inst_name = instruction.__name__
    _df = inst_df.query(f"stage == {stage_id} and instruction == '{inst_name.lower()}'")
    options = []
    ...
    for _, row in _df.iterrows():
        row = row.to_dict()
        if instruction is Backward and args.use_activation_checkpointing:
            # AC+CP: 5 recompute-forward + 7 backward configs
            options.append(
        ...
        elif instruction is Backward:
            # non-AC CP: 7 backward configs
            options.append(
...
def _write_results_non_cp(instructions, iteration, output_dir, args):
    """Write freqs/scheds files for non-CP mode."""
            ...
            if args.use_activation_checkpointing:
                if isinstance(knob, tuple):
                    ...
                    if inst_name == "backward" and len(knob) >= 5:
                        stage_cfgs_1f1b.append((inst_name, knob[1], knob[2], knob[3], knob[4]))
                    else:
                        attn_cfg = knob[1] if len(knob) > 1 else ""
                        mlp_cfg = knob[2] if len(knob) > 2 else ""
                        stage_cfgs_1f1b.append((inst_name, attn_cfg, mlp_cfg))
                else:
                ...
                if isinstance(knob, tuple):
                    ...
                    if inst_name == "backward":
                        ...
                        stage_cfgs_by_mb[inst.micro_batch_id][inst_name] = (inst_name, bwd_attn_cfg, bwd_mlp_cfg)
                    else:
                        attn_cfg = knob[1] if len(knob) > 1 else ""
                        mlp_cfg = knob[2] if len(knob) > 2 else ""
                    ...
                    freq = knob
                    stage_cfgs_by_mb[inst.micro_batch_id][inst_name] = (inst_name, "", "")
    ...
    f_freqs.write("]\n")
    f_cfgs.write("]\n")
    return f_freqs, f_cfgs


def _write_results_cp(instructions, iteration, output_dir, args):
    """Write freqs/scheds files for CP mode."""
    f_freqs = open(output_dir / f"freqs_pipeline_{iteration:05d}.py", "w")
    f_cfgs = open(output_dir / f"scheds_pipeline_{iteration:05d}.py", "w")
                ...
                if isinstance(knob, tuple):
                    ...
                    if inst_name == "backward" and len(knob) >= 13:
                        stage_cfgs_1f1b.append((
                            ...
                            knob[6], knob[7], knob[8], knob[9], knob[10], knob[11], knob[12],
                        ))
                    else:
                        ...
                        stage_cfgs_1f1b.append((inst_name, qkv_ar, qkv_ag, ao_ag, ao_ar, mlp_cfg))
                else:
                    raise ValueError(f"Invalid knob type: {type(knob)}")
            else:
                if isinstance(knob, tuple):
                    ...
                    if inst_name == "backward":
                        stage_cfgs_by_mb[inst.micro_batch_id][inst_name] = (
                            inst_name,
                            knob[1] if len(knob) > 1 else "",
                            knob[2] if len(knob) > 2 else "",
                            knob[3] if len(knob) > 3 else "",
                            knob[4] if len(knob) > 4 else "",
                            knob[5] if len(knob) > 5 else "",
                            knob[6] if len(knob) > 6 else "",
                            knob[7] if len(knob) > 7 else "",
                        )
                    else:
                        stage_cfgs_by_mb[inst.micro_batch_id][inst_name] = (
                            inst_name,
...
def main(args: Args) -> None:
    ...
    cp_mode = "fwd_qkv_ar" in inst_df.columns
    logger.info("Detected %s mode from CSV header.", "CP" if cp_mode else "non-CP")

    build_options = _build_options_cp if cp_mode else _build_options_non_cp
    ...
    ###################################
    write_results = _write_results_cp if cp_mode else _write_results_non_cp
    ...
    for iteration, result in enumerate(solver.run()):
        ...
        f_freqs, f_cfgs = write_results(instructions, iteration, output_dir, args)
