    ...
    def backward(
        ...
        for fused_idx, (op, basic_op_idxs) in enumerate(backward_ops):
            ...
            if comm_start == fused_idx:
                ...
                # else:
                #     current_stream.synchronize()
                # Synchronization in reduce_scatter_kv
                grad_comm_key, grad_comm_value = comm_op_bwd.fuser_forward(
                    [None], None, # read from global variables
                    basic_op_extra_inputs=[(None,)], 
                    basic_op_prev_ops=[None], 
        ...
        return (
            ...
            None,  # rotary_pos_emb
            None,  # attention_mask
            grad_comm_key,  # comm_input
            grad_comm_value,  # comm_input
...
class QKVPartitionFuser2:
    ...
    def __call__(
        ...
        if is_grad_enabled:
            forward_func = _QKVFuserAutogradFunction.apply
            args = []
        else:
            forward_func = _QKVFuserAutogradFunction.forward
            args = [None]
        args += (
            hidden_states,
            bias,
