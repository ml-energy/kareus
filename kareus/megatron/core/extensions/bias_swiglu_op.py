...
def fused_bias_swiglu_forward(
    ...
    """Compiled forward function for fused bias SwiGLU operation."""
    ...
    # Apply SwiGLU: split tensor in half along last dimension
    # SwiGLU(x) = SiLU(x1) * x2, where x1, x2 = split(x, 2, dim=-1)
...
def fused_bias_swiglu_backward(
    ...
    """Compiled backward function for fused bias SwiGLU operation."""
    # Add bias if provided (recreate forward computation)
    if bias is not None:
        x_plus_bias = x + bias
    else:
        x_plus_bias = x
    ...
    # Compute gradients
    # For SwiGLU: output = silu(x1) * x2
    # grad_x1 = grad_output * x2 * silu'(x1)
    # grad_x2 = grad_output * silu(x1)
...
class BiasSwigluOp(BasicOperation):
