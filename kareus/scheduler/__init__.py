...
def _parse_config_file(path: Path, use_activation_checkpointing: bool) -> List[List[ScheduleItem]]:
...
class PipelineCommScheduler:
    ...
    def __init__(self, configs_pipeline: Union[str, os.PathLike], pp_rank: int, num_microbatches: int, use_activation_checkpointing: bool = False) -> None:
        ...
        self._ac_mode: bool = bool(use_activation_checkpointing)
        parsed = _parse_config_file(self._config_path, self._ac_mode)
        ...
        self._schedule_items: List[ScheduleItem] = list(parsed[pp_rank])  # type: ignore[index]
    ...
    def on_instruction_begin(self, name) -> None:
        ...
        if self._ac_mode:
            ...
            expected = "forward" if (item.bwd_attn is None and item.bwd_mlp is None) else "backward"
            if expected != name:
                raise RuntimeError(
