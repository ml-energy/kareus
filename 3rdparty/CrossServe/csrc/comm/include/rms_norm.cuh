...
__global__ void rmsnorm_twoPassAlgo_e8(float4* __restrict__ i_output,
                                       const float4* __restrict__ i_input,
                                       const float4* __restrict__ weight,
                                       int m,
                                       int n,
                                       float epsilon) {
#pragma unroll
  for (int iter = 0; iter < m / gridDim.x; iter++) {
    int m_idx = blockIdx.x + iter * gridDim.x;
    ...
    auto input = i_input + offset;
    auto output = i_output + offset;
    ...
    for (int index = tid; index < n_8; index += bdimx) {
      ...
      h1->x = static_cast<half>(static_cast<float>(l1->x) * s_mean * static_cast<float>(g1->x));
      h1->y = static_cast<half>(static_cast<float>(l1->y) * s_mean * static_cast<float>(g1->y));
      h2->x = static_cast<half>(static_cast<float>(l2->x) * s_mean * static_cast<float>(g2->x));
      h2->y = static_cast<half>(static_cast<float>(l2->y) * s_mean * static_cast<float>(g2->y));
      h3->x = static_cast<half>(static_cast<float>(l3->x) * s_mean * static_cast<float>(g3->x));
      h3->y = static_cast<half>(static_cast<float>(l3->y) * s_mean * static_cast<float>(g3->y));
      h4->x = static_cast<half>(static_cast<float>(l4->x) * s_mean * static_cast<float>(g4->x));
      h4->y = static_cast<half>(static_cast<float>(l4->y) * s_mean * static_cast<float>(g4->y));
...
template <typename half, typename half2>
__inline__ __device__ void rmsnorm_device(float4* __restrict__ i_output,
                                          const float4* __restrict__ i_input,
                                          const float4* __restrict__ weight,
                                          int m,
                                          int n,
...
template <typename T>
bool rms_norm(T* __restrict__ output,
  ...
  if (std::is_same<T, nv_half>::value) {
    rmsnorm_twoPassAlgo_e8<nv_half, nv_half2>
        <<<grid, block, 0, stream>>>((float4*)output, (float4*)input, (float4*)weight, rows, columns, epsilon);
    return true;
