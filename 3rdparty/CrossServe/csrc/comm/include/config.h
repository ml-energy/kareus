...
struct vortexModelConfig {
  ...
  int model_layer = 80;
  int run_layer = 80;
  int allocate_kv_data_batch = 400;  // pages for 400 req averaging length 1024 (16*64, 64 will be
                                     // multiplied later) 16 tokens/page
  ...
  void setConfig(int gpu_num_,
                 int model_layer_,
                 int run_layer_,
                 int allocate_kv_data_batch_,
                 int model_head_dim_,
                 int model_gqa_,
                 int model_kv_heads_,
                 int model_qo_heads_,
                 int model_ff_dim_,
                 int model_hidden_dim_,
                 int frame_page_size_,
                 int max_batch_size_,
                 float rms_norm_eps_,
                 float rope_theta_,
                 int vocab_size_,
...
// static int ALLOCATE_KV_DATA_BATCH = 1500; //pages for 400 req averaging
// length 1024 (16*64, 64 will be multiplied later) 16 tokens/page
...
// // TODO: when supporting GQA and updating the above MODEL_{KV,QO}_HEADS,
// update the below static_assert
// // static_assert(MODEL_HEAD_DIM * MODEL_QO_HEADS * GPU_NUM ==
// MODEL_HIDDEN_DIM); static int KQV_N = MODEL_HIDDEN_DIM_PERGPU + 2 *
// MODEL_HIDDEN_DIM_PERGPU / MODEL_GQA;
...
// static size_t PAGE_MEM_SIZE = PAGE_SIZE * MODEL_KV_HEADS * MODEL_HEAD_DIM * 2
// * 2; static size_t MAX_PAGE_NUM = size_t(GPU_MEM / MODEL_LAYER /
// PAGE_MEM_SIZE);
