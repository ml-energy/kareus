...
enum class PllmLayout { ROW_MAJOR, COL_MAJOR };

inline std::string toString(PllmLayout layout) {
  switch (layout) {
    case PllmLayout::ROW_MAJOR:
      return "ROW_MAJOR";
    case PllmLayout::COL_MAJOR:
      return "COL_MAJOR";
    default:
...
enum class PllmDimension { ROW, COLUMN };

inline std::string toString(PllmDimension dim) {
  switch (dim) {
    case PllmDimension::ROW:
      return "ROW";
    case PllmDimension::COLUMN:
      return "COLUMN";
    default:
...
struct pllmTensor {
  ...
  pllmTensor() : ptr(nullptr), dim1(0), dim2(0), dimC(this->dim2), layout(PllmLayout::ROW_MAJOR) {}
  pllmTensor(T* ptr, size_t dim1, size_t dim2, PllmLayout layout)
      : ptr(ptr), dim1(dim1), dim2(dim2), dimC(this->dim2), layout(layout) {}
  ...
  pllmTensor(T* ptr, size_t length)
      : ptr(ptr), dim1(length), dim2(1), dimC(this->dim2), layout(PllmLayout::ROW_MAJOR) {}

  pllmTensor(T* ptr, int length) : ptr(ptr), dim1(length), dim2(1), dimC(this->dim2), layout(PllmLayout::ROW_MAJOR) {
    assert(length >= 0);
  }
  ...
  std::array<pllmTensor<T>, sizeof...(Lens)> splitTensor(PllmDimension dim, Lens... lens) const {
    ...
    if (totalLength != 2560) {
      assert((dim1 == totalLength) && "Subtensor length sum mismatches with the input tensor");
    }
    size_t i = 0;
    size_t offset = 0;
    std::array<pllmTensor<T>, sizeof...(Lens)> ret;

    ((ret[i++] = (static_cast<size_t>(lens) == 0
  ...
  inline std::array<pllmTensor<T>, N> splitTensor(PllmDimension dim,
                                                  const LensContainer<TLen, N>& lens,
                                                  TLen suffix = 0) {
    assert(sameDirection(dim));
  ...
  inline std::array<pllmTensor<T>, N> splitTensor(PllmDimension dim,
                                                  const LensContainer<TLen, N>& lens,
                                                  const LensContainer<TLen, N>& suffix) {
    assert(sameDirection(dim));
...
inline PllmLayout getMajorType(const std::string& input, int idx) {
  ...
  auto it = std::find_if(
      parts.begin(), parts.end(), [](const std::string& s) { return s == "RowMajor" || s == "ColumnMajor"; });
