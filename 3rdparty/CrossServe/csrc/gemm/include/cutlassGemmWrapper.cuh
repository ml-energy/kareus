...
struct CutlassGEMMWrapper : public BaseGEMMWrapper {
  ...
  CutlassGEMMWrapper()
      : problem_size(M, N, K), tensor_a_ref(), tensor_b_ref(), tensor_c_ref(), tensor_d_ref(), alpha(1), beta(0) {}
  ...
  void init(ElementComputeEpilogue beta_) override {
    ...
    spdlog::info("name:{} M:{}, N:{}, K:{}, a, b, c, d: {}, {}, {}, {}",
                 name,
                 M,
                 N,
                 K,
                 (size_t)tensor_a_ref.data(),
                 (size_t)tensor_b_ref.data(),
                 (size_t)tensor_c_ref.data(),
                 (size_t)tensor_d_ref.data());
    spdlog::info("lda, ldb, ldc, ldd: {}, {}, {}, {}", kLda, kLdb, kLdc, kLdd);
    beta = beta_;
    typename Gemm::Arguments arguments{
        problem_size, tensor_a_ref, tensor_b_ref, tensor_c_ref, tensor_d_ref, {alpha, beta}, split_k};
  ...
  void set_weight(ElementInputB* data_b) {
    ...
    if (!inited)
      return;  // kan: isInitilized not work if nrank != vnrank. use this inited can temporarily solve this problem.
    typename Gemm::Arguments arguments{
        problem_size, tensor_a_ref, tensor_b_ref, tensor_c_ref, tensor_d_ref, {alpha, beta}, split_k};
  ...
  bool checkResult() {
    ...
    gemm_device(problem_size, alpha, tensor_a_ref, tensor_b_ref, beta, tensor_c_ref, tensor_d_standard.device_ref());
    cudaDeviceSynchronize();
    tensor_d_standard.sync_host();

    ElementOutput* data_d = new ElementOutput[M * N];
    cudaMemcpy(data_d, tensor_d_ref.data(), sizeof(ElementOutput) * M * N, cudaMemcpyDeviceToHost);
