...
with open(
    f"{LOG_DIR}/benchmark/component_scaling_efficiency/ring_scaling_efficiency/ring_scaling_efficiency.json", "r"
) as f:
    data = json.load(f)
...
filtered_groups = {k: v for k, v in bs_seqlen_groups.items() if len(v) >= 2}
...
for idx, ((bs, seq_len), entries) in enumerate(sorted_groups):
    # Sort entries by world_size
    entries = sorted(entries, key=lambda x: x["ring_attn_world_size"])

    ring_attn_world_sizes = [entry["ring_attn_world_size"] for entry in entries]
    times = [entry["time"] for entry in entries]

    # Plot actual measurements
    axs[idx].plot(ring_attn_world_sizes, times, "o-", label="Actual")

    # Plot ideal scaling reference line
    if len(ring_attn_world_sizes) > 0 and len(times) > 0:
        base_ring_attn_world_size = ring_attn_world_sizes[0]
        base_time = times[0]
        ideal_times = [base_time / (w_size / base_ring_attn_world_size) for w_size in ring_attn_world_sizes]
        axs[idx].plot(ring_attn_world_sizes, ideal_times, "--", color="orange", label="Ideal Scaling")

    axs[idx].set_title(f"BS={bs}, Seq_len={seq_len} (Total={bs*seq_len})")
    axs[idx].set_xlabel("Ring Attention World Size")
...
plt.tight_layout()
plt.savefig(f"${LOG_DIR}/benchmark/component_scaling_efficiency/ring_scaling_efficiency/ring_scaling_efficiency.png")
plt.close()
