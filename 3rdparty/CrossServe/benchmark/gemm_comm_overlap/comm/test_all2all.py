...
if __name__ == "__main__":
    ...
    with open("all2all_{}.csv".format(freq), "w") as f:
        f.write(
    ...
    for sm_num in range(3, 31, 3):
        for block_size in [128, 256, 512, 1024]:
            os.system("mpirun --allow-run-as-root -np 4 build/test_comm {} {} > tmp 2>&1".format(sm_num, block_size))

            data = parse_output("tmp")
            max_time = max([data[i]["time"] for i in range(4)])
            total_energy = sum([data[i]["energy"] for i in range(4)])

            with open("all2all_{}.csv".format(freq), "a") as f:
                f.write(
                    "{},{},{},{},{},{},{},{},{},{},{},{}\n".format(
                        sm_num,
                        block_size,
