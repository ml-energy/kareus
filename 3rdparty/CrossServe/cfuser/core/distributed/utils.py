...
def generate_all_subsets(n):
    """
    Generate all subsets of a list of numbers from 0 to n-1.

    Parameters:
    n (int): The number specifying the range (0 to n-1).

    Returns:
    list: A list of all subsets represented as lists.
    """
    nums = list(range(n))
    return [list(subset) for r in range(1, n + 1) for subset in combinations(nums, r)]


def filter_continuous_sequences(subsets):
    """
    Filter out only the subsets that form continuous sequences.

    Parameters:
    subsets (list): A list of subsets (each subset is a list).

    Returns:
    list: A list of subsets that are continuous sequences.
    """
    continuous = []
    for subset in subsets:
        if all(subset[i] + 1 == subset[i + 1] for i in range(len(subset) - 1)):
            continuous.append(subset)
    return continuous
