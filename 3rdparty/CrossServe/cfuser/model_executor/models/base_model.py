...
from cfuser.config import InputConfig, ParallelConfig, RuntimeConfig
from cfuser.core.distributed.parallel_state import get_sequence_parallel_world_size
from cfuser.model_executor.base_wrapper import cFuserBaseWrapper
from cfuser.model_executor.layers import *
...
class cFuserModelBaseWrapper(nn.Module, cFuserBaseWrapper, metaclass=ABCMeta):
    wrapped_layers: List[cFuserLayerBaseWrapper]
    ...
    def __getattr__(self, name: str):
        ...
        try:
            ...
            raise AttributeError(f"module {type(self.module).__name__} has no " f"attribute {name}")
    ...
    def reset_activation_cache(self):
        for layer in self.wrapped_layers:
            if hasattr(layer, "activation_cache"):
                ...
                logger.info(f"layer {type(layer)} has no attribute " f"activation_cache, do not need to reset")
    ...
    def _wrap_layers(
        ...
        for name, module in model.named_modules():
            if isinstance(module, cFuserLayerBaseWrapper):
                continue
            ...
            for subname, submodule in module.named_children():
                ...
                if need_wrap:
                    wrapper = cFuserLayerWrappersRegister.get_wrapper(submodule)
                    ...
                    if additional_args is not {}:
                        ...
                        setattr(  # NOTE(lry89757) FluxTransformer2DModel follows this one
                            module,
                            subname,
                            wrapper(submodule),
                        )
                    # if isinstance(getattr(module, subname), cFuserPatchEmbedWrapper):
